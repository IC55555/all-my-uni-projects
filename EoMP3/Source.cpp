...
//	double w = 1;//параметр релаксации
	...
	{ //получение глобального номера по локальному
		int k;
		switch (j)
		{
		case 0:
	...
	void add_first()
	{
		...
		for (int i = 0; i < ncond1; i++)
		{
			...
			int flag;
			if (z_num_beg == z_num_end) flag = 0;//нижняя и верхняя
			else
			{
				if (y_num_beg == y_num_end) flag = 1;//передняя и задняя
				else flag = 2; //боковые
			}
			...
			{//нижняя или верхняя				
				...
				for (s = oyn[y_num_beg]; s < oyn[y_num_end]; s++)
				{
					...
					{
						//ставим в p и c для 0-3(нижних) узлов
			...
			{//передняя или задняя				
				...
				for (r = ozn[z_num_beg]; r < ozn[z_num_end]; r++)
				{
					for (p = oxn[x_num_beg]; p < oxn[x_num_end]; p++)
					{
						//ставим в p и c для 0 1 4 5(передних) узлов
						di[2 * get_global(r, s, p, 0)] = B;
						di[2 * get_global(r, s, p, 0) + 1] = B;
						di[2 * get_global(r, s, p, 1)] = B;
						di[2 * get_global(r, s, p, 1) + 1] = B;
			...
			{//левая или правая				
				...
				for (r = ozn[z_num_beg]; r < ozn[z_num_end]; r++)
				{
					for (s = oyn[y_num_beg]; s < oyn[y_num_end]; s++)
					{
						//ставим в p и c для 0 2 4 6(левых) узлов
						di[2 * get_global(r, s, p, 0)] = B;
						di[2 * get_global(r, s, p, 0) + 1] = B;
						di[2 * get_global(r, s, p, 2)] = B;
						di[2 * get_global(r, s, p, 2) + 1] = B;
	...
	void LU_mult_pr()
	{
		...
		for (int i = 0; i < q.size(); i++)
		{
			double s = 0; //переменная суммирования
	...
	void LU()
	{
		for (int i = 0; i < q.size(); i++)
		{
			double s1 = 0, s2 = 0, s3 = 0; //переменные суммирования
	...
	void decompositionLU()
	{
		...
		int N = 0;
		int N1 = 0;
		int NN = 0; // количество элементов в строке / столбце
		int number = 0;
		int number1 = 0;
		for (int i = 0; i < ig_pr.size() - 1; i++)
		{
			N = ig_pr[i + 1] - ig_pr[i]; // число элементов в строке

			//считаем матрицу U
			for (int j = ig_pr[i]; j < ig_pr[i + 1]; j++)
			{
				sum = 0;
				N1 = j - ig_pr[i]; // номер числа в столбце
				NN = ig_pr[i - N + N1 + 1] - ig_pr[i - N + N1]; // количество чисел в стро-ке

				if (i - N < i - N + N1 - NN) // первая строка < первого столбца
				{
					number = N1 - NN;
					number1 = 0;
				}
				...
				for (int k = number, k1 = number1; k < N1; k++, k1++)
					sum += ggl_pr[ig_pr[i - N + N1] + k1] * ggu_pr[ig_pr[i] + k];

				ggu_pr[j] = (ggu_pr[j] - sum) / di_pr[i - N + N1];
			}
			...
			{
				sum = 0;
				N1 = j - ig_pr[i]; // номер числа в строке
				NN = ig_pr[i - N + N1 + 1] - ig_pr[i - N + N1];

				if (i - N < i - N + N1 - NN) // первый столбец < первой строки
				{
					number = N1 - NN;
					number1 = 0;
				}
				...
				for (int k = number, k1 = number1; k < N1; k++, k1++)
					sum += ggl_pr[ig_pr[i] + k] * ggu_pr[ig_pr[i - N + N1] + k1];
				ggl_pr[j] = ggl_pr[j] - sum;
			}

			sum = 0;

			for (int k = 0; k < N; k++)
				sum += ggl_pr[ig_pr[i] + k] * ggu_pr[ig_pr[i] + k];
			di_pr[i] = di_pr[i] - sum;
		}
	}
	void mult_L_pr_on_vec(vector<double> vec, vector<double> &res)
	{
		...
		{
			res[i] = vec[i];
			int N = ig_pr[i + 1] - ig_pr[i]; // количество элементов в строке
			int col_one_elem = i - N; // номер столбца первого элемента
			for (int j = 0; j < N; j++)
	...
	void mult_U_pr_on_vec(vector<double> vec, vector<double> &res)
	{
		...
		{
			res[i] = di_pr[i] * vec[i];
			int N = ig_pr[i + 1] - ig_pr[i]; // количество элеметов в столбце
			int row_one_elem = i - N; // номер строки первого элемента
			for (int j = 0; j < N; j++)
	...
	void LU_sol()
	{
		//перегенерация в профильный
		generate_profil();
		//ЛУ-разложение для профильного
		LU();
		//decompositionLU();
		LU_mult_pr();
		LU_mult_obr();
		...
		cout << "nev=" << nev << endl;
		ofstream fout("results.csv", ios_base::app); // открываем файл для добавления информации к концу файла
		fout << nev << "\n";
		fout.close();
	}
	...
	void do_smth()
	{
		...
		end_time = clock(); // время работы программы
...
int main()
{
	SLAE slv = SLAE("net", "cond1", "cond2", 1);

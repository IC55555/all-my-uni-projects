...
struct border_cond_1
{
	int ni;
	double plastPressure;

	border_cond_1(int _ni = 0, double _plastPressure = 0.0) :
		ni(_ni), plastPressure(_plastPressure) {};
};

struct border_cond_2
{
	size_t ielem;
	size_t loc_node_1, loc_node_2;
	double tetta;

	border_cond_2(size_t _ielem = 0, size_t _loc_node_1 = 0, size_t _loc_node_2 = 2, double _tetta = 0.0) :
		ielem(_ielem), loc_node_1(_loc_node_1), loc_node_2(_loc_node_2), tetta(_tetta) {};
};
...
public:
	...
	vector<double> cax, cay, cnx, cny, cqx, cqy; //те же самые у узлы, только в цилиндрических координатах
	vector<int> areas;//массив областей
	int m; //количество узлов x
	int n;//количество узлов y
	...
	void read_net(string net_file)
	{
		...
		//перевод в декартовые координаты из цилиндрических

		ofstream fout("decard.txt");
		fout << n_x << ' ' << n_y << endl;
		for (int i = 0; i < n_x; i++)
		{

			ax[i] = cax[2*i] * cos(cax[2*i + 1]);//первый радиус второй угол
			fout << ax[i] << ' ';
		}
		fout << endl;
		for (int i = 0; i < (n_x - 1); i++)
	...
	double get_lyam(double hx, double hy, double x, double y, int p, vector<double>current)  //подсчет лямбды
	{
	...
	double get_f(double x, double y, double t)  //функция f(x)
	{
		return x * x * x * x + y * y * y * y - 12 * x * x - 12 * y * y;
	}

	double get_Ug(double x, double y, double t) //возведение в квадрат
	{
		return x * x + y * y;
	}
	...
	void make_local_G(int p, vector<double>current) //создание окальной матрицы G в плотном формате
	{
		double hx = x[p + 1] - x[p];
		double hy = y[p + 1] - y[p];
		double lyam1 = get_lyam(hx, hy, x[p], y[p], p, current);
		double lyam2 = get_lyam(hx, hy, x[p + 1], y[p + 1], p, current);
	...
	void make_local_vec(int p, double t) //создание локального вектора правой части
	{
		...
		hy = y[p + 1] - y[p];
		f1 = get_f(x[p], y[p], t);
		f2 = get_f(x[p + 1], y[p + 1], t);
		b_loc[0][0] = hx * (2 * f1 + f2) / 6;
		b_loc[0][1] = hx * (f1 + 2 * f2) / 6;
		b_loc[1][0] = hy * (2 * f1 + f2) / 6;
		b_loc[1][1] = hy * (f1 + 2 * f2) / 6;
	}
	...
	{
		b[p] += b_loc[0][0] + b_loc[1][0];
		b[p + 1] += b_loc[0][1] + b_loc[1][1];
	}
	...
	void add_first(double t) //начальные условия
	{
		...
		for (int i = 0; i < ncond1; i++) //0 то левый край, если 1(или что то ещё) то правый край
		{
			...
			int ay_num = cond1[i];
			if (ay_num == 0) y_num = 0;
			else y_num = count_y - 1;

			Adi[x_num] = B;
			d[x_num] = B * get_Ug(x[x_num], y[y_num], t);
		}
		for (int i = 0; i < ncond1; i++) //0 то левый край, если 1(или что то ещё) то правый край
		{
			int ax_num = cond1[i];
			...
			Adi[y_num] = B;
			d[y_num] = B * get_Ug(x[x_num], y[y_num], t);
		}
	}
	//ДОДЕЛАТЬ ВТОРЫЕ КРАВЕВЫЕ
	//void add_second(double t) //начальные условия
	//{
	//	long B = 1e+10;
	//	int x_num;
	//	int y_num;
	//	for (int i = 0; i < ncond2; i++) //0 то левый край, если 1(или что то ещё) то правый край
	//	{
	//		
	//		int ax_num = cond1[i];
	//		if (ax_num == 0) x_num = 0;
	...
	//int add_second(SLAE &MS) // учет вторых краевых MS.b - вектор вклада от краевых
	//{
	//	 std::vector<double> b_loc(2);
	//	 int NS2 = cond2.size();
	//	 for (int i = 0; i < NS2; i++)
	//	 {
	//	 int s2_id = cond2[i].first;
	//		for (int j = 0; j < cond2[i].second.size()- 1; j++)
	//		{
	//	      int node_id1 = cond2[i].second[j],
	//	      node_id2 = cond2[i].second[j + 1];

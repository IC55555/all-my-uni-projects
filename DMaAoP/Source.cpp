...
vector<vector<double>> source(2, vector<double>(3)), receiver(6, vector<double>(3));
...
{
	double V = I / (2 * sigma * sigma * M_PI) * ((1 / R(M, B) - 1 / R(M, A)) - (1 / R(N, B) - 1 / R(N, A)));
	return V;
}

double find_sigma(double V, double I, point A, point B, point M, point N)
{
	double sigma = I / (2 * V * M_PI) * ((1 / R(M, B) - 1 / R(M, A)) - (1 / R(N, B) - 1 / R(N, A)));
	return sqrt(sigma);
}
...
void init(point M1, point N1, point M2, point N2, point M3, point N3, point A2, point B2)
{
	source[0] = get_vector(A2);
	source[1] = get_vector(B2);
...
void main()
{
	...
	I_cur = I_true;
	srand(time(0));
	double div = 1. -0.1 + 0.01 * (rand() % 21);
	straight_1();

	point A = get_point_from_source(source[0]);
	point B = get_point_from_source(source[1]);

	point M1 = get_point_from_source(receiver[0]);
	point N1 = get_point_from_source(receiver[1]);

	point M2 = get_point_from_source(receiver[2]);
	point N2 = get_point_from_source(receiver[3]);

	point M3 = get_point_from_source(receiver[4]);
	point N3 = get_point_from_source(receiver[5]);


	cout << "sigma1 = " << find_sigma(V_true[0] * div, I_true[0] * div, A, B, M1, N1) << " div = " << div << endl;
	cout << "sigma2 = " << find_sigma(V_true[1] * div, I_true[0] * div, A, B, M2, N2) << " div = " << div << endl;
	cout << "sigma3 = " << find_sigma(V_true[2] * div, I_true[0] * div, A, B, M3, N3) << " div = " << div << endl;

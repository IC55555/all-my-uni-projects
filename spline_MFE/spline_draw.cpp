...
// Рисование картинки ^^
void FEM_spline::draw(unsigned int width, unsigned int height, unsigned int num_isolines, bool need_grid,
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < (int)height; i++)
    {
        ...
        for (unsigned int j = 0; j < width; j++)
        {
            ...
            sol_grid[i][j] = get_spline_solution(min_x + step_x * (double)j, min_y + step_y * (double)i);
        }
    }
    ...
    for (unsigned int i = 0; i < height; i++)
    {
        for (unsigned int j = 0; j < width; j++)
        {
            ...
            // Spline hack
            //if(curr_u > max_val) curr_u = max_val;
            //if(curr_u < min_val) curr_u = min_val;
